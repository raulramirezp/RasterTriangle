    ...
    for( int x = -relativeGrid; x <relativeGrid; x++){
      for( int y = -relativeGrid;  y < relativeGrid; y++){
        ...
        if ( (e12 >= 0 && e23 >= 0 && e31 >= 0) || (e12 < 0 && e23 < 0 && e31 < 0) ){
          ...
          noStroke();
          float[] colors = { w1*255, w2*255, w3*255 };
          if( antialising ) {
            colors = antialising(x,y);
...
float[] antialising(float x, float y){
  int numSubpixels = 4;
  float e23 = 0, e31 = 0, e12 = 0;
  float w1, w2, w3 = 0, DoubleAreaOfV1V2V3 = 0;
  float[] colors = {0,0,0};
  for(float subpixelx = 0; subpixelx < 1; subpixelx += (float)1/numSubpixels){
    for(float subpixely = 0; subpixely < 1; subpixely += (float)1/numSubpixels){
